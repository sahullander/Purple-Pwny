...
def findModules(service, details, port, host):
    ...
    search = 'search ' + service + ' -S ' + details + ' type:exploit && ' + exploitRanks + ' -o ' + outFile
    try:
...
def main(args):
	...
	if args.r:
		index = 0
		while index <= (len(args.r)-2):
			exploitRanks.append("rank:" + args.r[index] + " || ")
			index += 1
		exploitRanks.append("rank:" + args.r[len(args.r)-1]) # should add the last arg to list
	else:
	...
	print("--> Exploit ranks command: " + exploitRanks)
	global nm
	global client
	global console
	global bfServices
	...
	f.write("--> Exploit ranks command: {0}\n\n".format(exploitRanks))
	f.write('------ Your Host ------\n')
	f.write('Host IP: {0} \n'.format(IP))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--b", type=str, nargs=2, default=[userList,passList], help="Specify filepaths for bruteforce users list [arg1] and passwords list [arg2]. (--b /home/users.txt /home/pass.txt)")
